import os
import json
# Add langchain_community import for Dashscope Qwen
...
class LLMModel(BaseModel):
    ...
    def has_json_mode(self) -> bool:
        ...
        if self.is_deepseek() or self.is_gemini() or self.is_dashscope():
            return False
    ...
    def is_dashscope(self) -> bool:
        """Check if the model is a Dashscope/Qwen model"""
        return self.provider == ModelProvider.DASHSCOPE
...
def get_model(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | ChatOllama | GigaChat | None:
    ...
    # Add Dashscope qwen-max here
        ...
        return ChatTongyi(model=model_name,  # 应为 "qwen-max"
