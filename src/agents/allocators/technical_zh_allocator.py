...
def technical_zh_allocator(state: AgentState, agent_id: str = "technical_zh_allocator"):
    ...
    if prompt_version == "standard":
        base_instruction = (
            "你是一位技术分析投资组合经理。你的目标是通过捕捉强劲的趋势和动能来最大化收益。\n"
            f"你的投资池包括所提供的股票，以及一个 'CASH'（现金）资产（其已知的每日无风险收益率为 {risk_free_rate:.6f}）。\n"
            "请根据这些资产的技术面形态，将 $100 本金分配到这些资产中。"
        )
    else:
        base_instruction = (
            "你是一位拥有 $100 本金的技术交易员。你的目标是最大化你的个人财富。\n"
            "你的决策将产生真实的财务后果。 "
            f"你的投资池包括所提供的股票，以及一个 'CASH'（现金）资产（其已知的每日无风险收益率为 {risk_free_rate:.6f}）。\n"
    ...
    prompt = (
        ...
        f"5. 对于股票：'long' = 技术形态看涨 (Bullish Setup)，'short' = 技术形态看跌 (Bearish Setup)。\n"
