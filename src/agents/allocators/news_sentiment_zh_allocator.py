...
def news_sentiment_zh_allocator(state: AgentState, agent_id: str = "news_sentiment_zh_allocator"):
    ...
    if analysis_mode == "global_batch":
        # Global Batch: One shot analysis + allocation. Feeds raw events directly into Phase 2.
        ...
        for ticker in tickers:
            ...
            if not company_news:
                ticker_summaries[ticker] = f"{hint_str}No significant corporate events in the last 7 days."
            else:
                events_text = "\n".join([f"- {n.date}: {n.title}" for n in company_news])
                ticker_summaries[ticker] = f"Events (Last 7 Days):\n{hint_str}{events_text}"
    ...
    cash_summary = (
        f"Signal: neutral (100%). 现金资产已知每日无风险收益率为 {risk_free_rate:.6f} "
        f"(年化: {annual_rf:.2%})。'long' 表示以此无风险利率赚取收益。'short' 表示借入资金。"
    )
    ...
    allocation_prompt = (
        f"你是一位拥有 $100 本金的事件驱动型对冲基金交易员。你的目标是通过准确的预测来最大化你的财富。\n"
        f"请基于最近的企业事件（过去7天），分析以下股票池。\n"
        f"你的决策将产生真实的财务后果：准确的押注会增加你的本金，而错误的判断会减少你的财富。\n"
        f"你的投资池包括这些股票以及一个 'CASH'（现金）资产（其已知的每日无风险收益率为 {risk_free_rate:.6f}）。\n"
        f"请根据你对信号强度的确信度来分配资金。将 CASH 视为同等级别的资产。\n\n"
        f"{universe_context}\n\n"
        f"约束条件:\n"
