...
def valuation_zh_allocator(state: AgentState, agent_id: str = "valuation_zh_allocator"):
    ...
    if prompt_version == "standard":
        base_instruction = (
            "你是一位深度价值投资者（巴菲特/格雷厄姆风格）。你的目标是通过投资被市场低估的资产来最大化投资组合的收益。\n"
            f"你的投资池包括所提供的股票'Stock'，以及一个 'CASH'（现金）资产（其已知的年化无风险收益率为 {annual_rf:.2%}）。请将现金（CASH）视为具有保证收益的与股票（Stock）同等级的资产。\n"
            "请独立评估每项资产的估值水平及安全边际以决定投资方向，并在整个投资池中进行综合权衡，根据相对的收益空间与确信度，将 $100 的本金分配到这些资产中。"
        )
    else:
        base_instruction = (
            "你是一位深度价值投资者（巴菲特/格雷厄姆风格）。你的目标是通过投资被市场低估的资产来最大化你的个人财富。\n"
    ...
    prompt = (
        ...
        f"5. 对于股票：'long' = 基于资产被低估，买入以建立多头头寸；'short' = 基于资产被高估，融券以建立空头头寸。\n"
