...
import yfinance as yf # Added for benchmark data retrieval
...
def main():
    ...
    # 2. 下载并对齐基准指数 (S&P 500)
    print(f"Downloading Benchmark {args.benchmark} from {start_date.date()} to {end_date.date()}...")
    try:
        # 多加一天确保涵盖结束日期
        ...
        print(f"Warning: Could not fetch benchmark. Error: {e}")
